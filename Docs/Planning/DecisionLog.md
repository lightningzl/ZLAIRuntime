...
## 2026-09-04：将静态 NPC 内容管线与动态世界认知拆分为 M13/M14

决定：
M13 只实现 Persona Asset、DataTable/DataRegistry、JSON 导入导出和 NPC Spawner；M14 再实现世界规则、动态事实、个人 Knowledge/Belief 与 LLM 背景反馈。Persona 是可编辑的静态起点，个人经历和世界认知仍是 UE 运行时事实。

原因：
- 内容生产管线需要先稳定 ID、数据模型和投放方式，才能安全服务多个场景和 NPC。
- 把动态世界事实直接写进全局背景会破坏个人感知隔离，使 NPC 假装知道未感知事件。
- 两部分同时实现会混合编辑器、DataRegistry、运行时传播和协议演进，难以独立验收。
