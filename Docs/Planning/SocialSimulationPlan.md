...
在既有 4 NPC 社会沙盒上，优先提升“像有个人经历和社会关系的人”的反馈质量。攻击、威胁、道歉、后退、报告和交易尝试必须形成按 NPC 个人感知隔离的持续后果；商人、平民和守卫以不同身份、风险和利益作出连续且可见的反应。

- **玩家可操作成果**：可以连续攻击、威胁、道歉、后退、求助或尝试与商人交易，观察既往行为如何改变后续互动。
- **屏幕可见成果**：商人、平民和守卫由 LLM 基于各自个人事实、身份、关系、目标和风险形成不同的应对；可能表现为警告、拒绝交易、回避、呼救、介入或缓和，但不预写固定结果，且台词、立场和行动一致。
- **最小闭环**：个人感知的事件 -> UE 有界社会事实/关系更新 -> LLM 基于个人 Context 判断高层策略 -> UE 校验的行动与公开互动结果 -> 后续个人记忆。
- **完成标志**：同一攻击在首次、重复、停止/道歉后的回应可理解地不同；未目击者不泄露事实；真实模型、Stub 和离线路径均呈现连续的角色差异。
- **历史定稿**：[Milestone12.md](../Milestones/Milestone12.md)。

### Milestone 13：NPC 内容配置、批量管理与场景投放

把 NPC 人物配置从项目级 JSON 预设升级为插件可复用的内容生产管线。Persona Asset、DataTable 行、DataRegistry、单条/批量 JSON 和场景 Spawner 必须使用同一数据语义；策划可配置身份、背景摘要、性格、表达、目标、初始关系和即时状态，并把它安全地交给现有 Decision Context。

- **玩家可操作成果**：在编辑器为 NPC 选择 Persona Asset 或 DataRegistry ID，导入/导出单条或批量 JSON，在固定位置放置 Spawner 并运行场景。
- **屏幕可见成果**：生成的 NPC 类、名称、出生位置和人物差异正确；配置失败不生成半初始化角色，并有明确反馈。
- **最小闭环**：Asset/DataTable/JSON -> DataRegistry 或直接引用 -> Spawner/角色初始化 -> Inspector/Decision Context。
- **边界**：不修改 UE/Python 协议，不把配置作为运行时记忆，不实现动态世界事实、谣言、个人世界认知或完整角色编辑器。
- **完成标志**：单条/批量导入导出、ID 覆盖/新增、DataRegistry 下拉、两种 Spawner 方式和 M12 回归均可验证。

### Milestone 14：个人世界认知与动态背景反馈

在 M13 的静态 Persona 基础上，增加“世界真实状态”与“每个 NPC 的已感知知识/信念”的明确分层。提供场景级 World Context Asset/JSON 导入导出和插件 Setting，用于选择受控世界规则、势力背景与初始公开常识；世界变化只能经 UE 权威事件、感知、确认报告或受限传播进入一个 NPC 的知识；LLM 只能看当前 NPC 有界的世界认知和个人立场。

- **玩家可操作成果**：触发或切换场景级世界事件，例如警戒、宵禁、灾害或势力冲突，并继续与不同 NPC 互动。
- **屏幕可见成果**：守卫、商人、平民对同一世界变化有不同且连续的反馈；不知道事件的 NPC 不会假装知道，传闻会显示为不确定而非已确认事实。
- **最小闭环**：世界规则/动态事实 -> 个人感知、报告或传播 -> 有界 Knowledge/Belief -> Decision Context -> LLM 计划 -> UE 实际结果与后续认知变化。
- **边界**：不实现全局全知、自动事实抽取、无限谣言链、知识图谱、长期 SaveGame 或模型自行确认世界事件；World Context 配置不能直接把动态事件写成所有 NPC 已知事实。
- **完成标志**：同一事件的确认者、传闻接收者与未获知者产生不同 Context/计划；来源、可信度、时效、反驳和遗忘均可检查，且不泄露其他 NPC 私有认知。
...
| 6 | Milestone 12 | 连续攻击、缓和、求助或尝试交易 | NPC 记忆个人遭遇，商人/平民/守卫形成连续且不同的社会后果 |
| 7 | Milestone 13 | 配置、批量导入和投放 NPC | Persona Asset、DataRegistry 与 Spawner 驱动可见 NPC 差异 |
| 8 | Milestone 14 | 触发动态世界变化并观察不同 NPC | 每个人只按自己的世界认知、来源与立场反馈 |
