...
目标：在保持 UE 请求入口和模块边界稳定的前提下，由 Python Service 通过 OpenAI Responses API 生成真实、非流式的单轮 NPC 对话回复，并建立可配置、可测试、可诊断且不泄露密钥的 Provider 边界。
...
- 接受 `provider` 为 `stub` 或 `openai`，不绑定模型名称。
...
- 保留 Stub Provider 作为显式离线开发模式；运行模式默认使用 OpenAI Provider。
- 首个真实 Provider 使用官方 OpenAI Python SDK 和 Responses API。
- 模型通过环境配置提供；初始默认值为 `gpt-5.6-luna`，不得写入 UE 协议。
- API Key 仅从 `OPENAI_API_KEY` 读取，不接受请求字段、UE Config 或仓库文件中的密钥。
...
| 配置 | 必填 | 初始约定 |
| --- | --- | --- |
| `ZL_DIALOGUE_PROVIDER` | 否 | 默认 `openai`；显式设置为 `stub` 可离线运行 |
| `OPENAI_API_KEY` | OpenAI 模式必填 | 仅从进程环境读取 |
| `ZL_OPENAI_MODEL` | 否 | 默认 `gpt-5.6-luna` |
| `ZL_OPENAI_TIMEOUT_SECONDS` | 否 | 正数；必须小于 UE 外层请求超时 |
| `ZL_OPENAI_MAX_OUTPUT_TOKENS` | 否 | 正整数并设置合理硬上限 |
...
- 不维护多轮对话上下文，不使用 OpenAI 托管的会话状态或 `previous_response_id`。
...
| `M2-A02` | Python Service 在有效 OpenAI 配置下可启动；OpenAI 模式缺少或包含无效配置时给出明确、脱敏的失败信息。 |
| `M2-A03` | 使用合法 JSON 调用 `/v1/dialogue`，OpenAI 模式返回 `200`、非空 `reply` 且结构符合协议。 |
| `M2-A04` | `request_id` 和 `npc_id` 在真实 Provider 请求链路中保持一致，成功响应的 `provider` 为 `openai`。 |
...
| `M2-A07` | UE 能继续解析 `stub` 与 `openai` 成功响应，并对新增错误码保持兼容。 |
...
`M2-02` 至 `M2-06` 的实现和适用自动化验证全部完成后，里程碑进入 `验收中`；完成 `M2-07` 并为 `M2-A01` 至 `M2-A10` 留下可复查证据后，Milestone 2 才能标记为 `已完成`。
