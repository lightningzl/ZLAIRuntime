...
目标：在保持 UE 请求入口和模块边界稳定的前提下，由 Python Service 通过 Kimi 开放平台 Chat Completions API 和 `kimi-k3` 生成真实、非流式的单轮 NPC 对话回复，并建立可配置、可测试、可诊断且不泄露密钥的 Provider 边界。
...
- 模型通过环境配置提供；初始默认值为 `kimi-k3`，不得写入 UE 协议。
