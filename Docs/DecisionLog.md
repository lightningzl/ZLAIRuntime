...
决定：
Milestone 2 的首个真实 LLM Provider 从 OpenAI Responses API 切换为 Kimi 开放平台的 `kimi-k3`。Python Service 使用 Kimi 官方文档支持的 OpenAI 兼容 Python SDK，通过 `https://api.moonshot.ai/v1` 的 Chat Completions API 发起请求；密钥仅从 `MOONSHOT_API_KEY` 读取。协议成功响应使用逻辑标识 `kimi`，但 UE、Route 和 Dialogue Service 仍不依赖具体 SDK 类型。
...
- [Kimi Account and Payments](https://platform.kimi.ai/docs/guide/account-and-payments)
