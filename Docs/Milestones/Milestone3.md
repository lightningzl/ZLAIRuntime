...
- 为 NPC 人格、世界状态、对话消息和完整上下文增加协议结构体，并严格按归档时的 v1 协议序列化。
...
- Endpoint 和响应结构保持不变；请求新增可选 `context`。
...
`M3-02` 至 `M3-06` 的实现和适用自动化验证全部完成后，里程碑进入 `验收中`；完成 `M3-07` 并为 `M3-A01` 至 `M3-A10` 留下可复查证据后，Milestone 3 标记为 `已完成`。

验收记录见 [Milestone3Validation.md](../Validation/Milestone3Validation.md)。Milestone 3 已于 2026-07-27 完成；本文件为归档定稿，不随当前协议和实现继续演进。
