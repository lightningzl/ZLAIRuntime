...
- 模型通过环境配置提供；完成时默认值为 `kimi-k2.6`，不得写入 UE 协议。
...
| `ZL_KIMI_TIMEOUT_SECONDS` | 否 | 默认 20 秒，必须小于 UE 外层请求超时 |
| `ZL_KIMI_MAX_OUTPUT_TOKENS` | 否 | 默认 256，硬上限 4096 |

上述完成值由 Milestone 2 实现与验收固定；历史验证证据以 [Milestone2Validation.md](../Validation/Milestone2Validation.md) 为准。
