...
| 下一候选任务 | `M12-T02` Python v2 决策质量验证 |
| 已知阻塞 | 无；Decision v2 已获确认但尚未实现 |
...
- UE 已权威维护 NPC 生命、防卫、失能、冲突立场、受击私有 Observation 和既有 Relationship/Instant State；当前 Kimi、Stub 和离线降级均使用单 NPC Decision v1。Decision v2 已完成协议确认，尚未实现。
- 当前体验缺口是已感知的攻击、缓和、报告和商人互动尚未稳定提供给 LLM 足够的个人事实，使其形成连续、角色差异化的社会判断；M12 将以此为唯一体验重点。

## 当前执行边界

- `/v1/decision` 保持现状；已确认新增 `/v2/decision`，以开放高层计划、`social_situation` 与请求级有限能力实例承载角色判断。实现前不改变 v1 运行路径。
...
- M12 采用 Python 决策质量优先顺序：先以受控个人情境快照验证 Kimi/Stub 的计划质量，再实现 UE Context、执行器和表现。
