...
| 里程碑状态 | `已完成` |
| 当前活动任务 | 无 |
| 下一候选任务 | Milestone 11：可配置角色与 NPC 技术验证（尚未开始） |
| 已知阻塞 | 无 |
...
- 后续候选为最小化的配置驱动技术 Demo：少量预设玩家角色和 NPC 通过本地 JSON 导入/导出驱动名称、颜色、出生位置与既有人物/关系；AI 生成的候选 JSON 必须经 UE 校验后才可加载。仅在现有攻击校验成功后接入 `Variant_Combat` 的攻击/受击表现，不扩展协议或完整战斗系统。
...
- Milestone 11 仅已写入长期计划，尚未成为当前实施范围；开始前需将 M10 当前范围归档并建立独立的 M11 任务板与验收标准。
