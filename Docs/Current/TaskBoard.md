...
| `M6-07` | `已完成` | Important NPC 报告纵向切片 | 5 个 Important NPC、Witness→Guard 数据传播、Authority 更新和 Intent 回调 | `M6-06` | 无界面场景覆盖 `M6-A08`、`M6-A09`，不声称具体行为执行已实现 |
| `M6-08` | `已完成` | Inspector 与有界性能指标 | Event Chain、关系、Faction、Long Memory 快照及 120+5 聚合基准 | `M6-07` | 满足 `M6-A10` 的可观察性、性能记录和安全输出要求 |
| `M6-09` | `进行中` | 最终回归与验收记录 | UE/Python 回归、Target 编译、自动化、无界面集成和 Validation 文档 | `M6-08` | `M6-A01` 至 `M6-A10` 均有实际、可复查证据 |
