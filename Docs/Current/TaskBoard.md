...
- 里程碑状态：`实施中`
- 当前活动工作包：`M7-T05` 最小玩家行为解析与执行
- 下一工作包：`M7-T06` 对话/动作气泡与感知开发视图
...
| `M7-T03` | Speech/Action Event 与个人 Observation | 有界事件数据、说话和行为分离、逐 NPC Observation、生命周期与容量限制 | `M7-T01` | `已完成` | 支撑 `M7-A04` 至 `M7-A06` 的纯数据闭环 |
| `M7-T04` | 定向视觉与分级听觉 | 视野角/距离过滤、四种说话模式、InEar 约束、听见与目标判断分离 | `M7-T03` | `已完成` | 满足 `M7-A04`、`M7-A05`、`M7-A06` |
| `M7-T05` | 最小玩家行为解析与执行 | Face、Approach、MoveAway、Stop 白名单，目标校验，Started/Completed/Rejected 结果 | `M7-T01`, `M7-T02`, `M7-T03` | `进行中` | 满足 `M7-A07` |
