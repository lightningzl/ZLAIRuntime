...
| `M4-03` | `已完成` | SQLite Repository 与配置 | Schema 初始化、索引、连接/事务、数据库路径和检索预算配置、临时库测试 | `M4-01` | 初始化和重启可重复；数据库不入库；Repository 外无 SQL |
| `M4-04` | `已完成` | Memory Service 与历史合并 | Memory 内部类型、隔离检索、稳定排序、预算、重叠消除和幂等写入 | `M4-02`、`M4-03` | 满足隔离、顺序、预算、成功后写入、重复请求和失败回滚约束 |
...
| `M4-09` | `已完成` | 补全 SQLite 数据库设计文档 | Schema、字段、约束、索引、事务、生命周期和维护边界说明 | `M4-03` | 设计文档与实现一致，入口链接有效，不改变协议或运行行为 |
...
验证记录（2026-07-31）：新增 SQLite Repository、不可变持久化类型和供应商无关 Repository 接口；Schema v1、范围索引、WAL、显式事务、失败回滚、全局 `request_id` 幂等和脱敏错误均已实现。Repository/Settings 专项 32/32、Python 完整测试 124/124、`pip check` 与字节码编译通过；应用代码扫描确认 SQL 与 `sqlite3` 仅存在于 SQLite Repository，忽略规则覆盖数据库及伴随文件；`M4-A03` 已验证。
...
验证记录（2026-07-31）：新增与 SQLite 解耦的 Memory Service、合并历史与消息内部类型；Service 对 Repository 结果再次执行 scope/NPC 隔离、稳定排序和预算截断，并使用最长精确角色/内容边界重叠保留客户端快照。Context Builder 可显式接收合并历史且只追加一次当前输入；Memory/Builder/Repository 专项 35/35、Python 完整测试 139/139 与字节码编译通过；`M4-A04`、`M4-A05` 已验证。
...
验证记录（2026-07-31）：Dialogue Service 已按“读取→合并→单次生成→合法响应→完整写入”接线，无 Memory 路径不调用 Memory Service；读取、Provider、非法回复和写入失败均不产生错误轮次。应用启动/关闭负责 Repository 生命周期，数据库异常映射为脱敏 `500 internal_error`；日志只记录允许的关联与状态元数据。编排/API/日志专项 46/46、Python 完整测试 154/154、`pip check`、字节码编译和依赖扫描通过；`M4-A06`、`M4-A07` 已验证。
...
验证记录（2026-07-31）：Repository 新增只返回聚合数字的统计和按精确 scope/NPC 事务清理；本机维护命令要求清理确认，不注册 HTTP Route，不输出标识、路径或正文。默认数据库固定在忽略的 `PythonService/data`，Settings 表示隐藏路径；README 已补充环境、配置、Memory 语义和维护用法。Repository/Settings/Maintenance 专项 44/44、Python 完整测试 166/166、`pip check`、字节码编译、命令入口、数据库跟踪和安全扫描通过；`M4-A08` 已验证，`M4-A01` 的 Python 部分通过并等待 M4-07 UE 完整回归。
...
验证记录（2026-07-31）：新增固定脱敏的 `ZL.AI.DialogueMemoryDemo <seed|recall|other_scope|other_npc>`，仅记录请求关联、场景、匹配布尔值和长度；本地 Stub 集成扩展为旧入口、Context、连续 Memory、不同 scope、不同 NPC、服务端失败及非法 Context/Memory 单次回调。`ZLEditor Win64 Development` 真实增量编译成功，完整 `ZLAIRuntime` 自动化 9/9 通过；临时库聚合统计为 4 轮、2 个 scope、3 个 scope/NPC 分区，非法 Memory 未产生 HTTP 或写入。无界面 Game seed 演示收到 Stub 成功回调；Stub 为固定文本，因此语义标记预期留待 M4-08 真实 Kimi 验收。`M4-A01`、`M4-A09` 已验证。
...
### M4-09：补全 SQLite 数据库设计文档

- 新增数据库设计唯一来源，记录默认路径与覆盖方式、Schema 版本和应用生命周期。
- 记录 `dialogue_memory_turns` 的完整 DDL、字段语义、唯一约束和范围检索索引。
- 记录最近历史读取、成功后完整写入、幂等、统计、精确清理和失败回滚语义。
- 记录连接 PRAGMA、明文数据安全边界、明确不持久化的数据和实现文件映射。
- 从项目入口、架构、Python 模块说明和 Service README 链接数据库设计文档。

验证记录（2026-08-03）：逐项对照 `sqlite_repository.py`、`settings.py`、Memory 内部类型和维护入口确认文档与 Schema v1 实现一致；相对链接、README 锚点和 `git diff --check` 通过。仅修改文档，未执行代码测试；协议和运行行为未改变。
