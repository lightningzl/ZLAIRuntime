...
- 当前阶段：已完成
- 最后更新：2026-09-01
- 结论：`M8-A01` 至 `M8-A10` 全部通过；本地实现、Stub/失败纵向闭环、真实 Kimi 自由输出闭环和全量回归均有实际证据
...
- 分支：`main`（由 `codex/milestone-8` 合并）
...
| 真实 Kimi 场景闭环 | 用户明确授权后启动 Kimi Service，默认沙盒地图执行 `-ZLSandboxDecisionKimiSmoke` | 通过 | UE→Python→Kimi→UE 两次合法自由输出均为 HTTP 200 / `provider=kimi`：一次 Speech + `face_target` 被接受并推进状态版本（2153 ms），一次 Speech-only 以合法 `NoTool` 分支退出码 0（6239 ms）；未记录上下文、回复正文或凭据 |
| Python 最终全量回归 | 在 `PythonService` 执行 `.venv/Scripts/python -m pytest` | 通过 | 收集 194 项，194 项通过，耗时 1.52 秒；新增真实模型输出形状约束回归 |
...
| UE Social 最终全量回归 | Kimi 验收器修正后，NullRHI + 内存 DDC 执行 `Automation RunTests ZL.Social` | 通过 | 收集 24 项，24 项全部 `Success`，包含 Tool Registry、个人上下文和 NPC Decision Action，退出码 0 |
...
- Python 最终回归已覆盖 Dialogue、Memory、Decision Schema、Context Builder、Service、Stub/Kimi Planner 映射与错误分类；真实 Kimi 网络调用另由默认地图端到端证据覆盖。
...
- `AZLSocialSandboxGameMode` 已把通用 Registry 校验后的 Face、Approach、MoveAway 和 Stop 交给 Guard Gameplay Handler；只有接受结果才推进权威状态并产生适用的世界副作用。
- Decision 协议已在用户明确确认后同步；现有 Dialogue 字段和纯文本语义未改变，也没有从 Dialogue 文本推断 Gameplay 动作。
- `FZLSocialToolRegistry` 已完成纯规则权威校验并只在接受后提交幂等与冷却状态；NPC 面向、移动和停止副作用已由场景纵向闭环验证。

## 验收进度

| ID | 当前证据 | 状态 |
| --- | --- | --- |
| `M8-A01` | Python 194/194、UE Social 24/24、UE AI Runtime 13/13 全部通过；ZLEditor Win64 Development 最终编译成功 | `通过` |
...
| `M8-A05` | 获明确外发授权后，真实 Kimi 经默认场景分别返回含 `face_target` 和 Speech-only 的合法自由 Decision；UE 均按协议处理，最终烟测退出码 0，不依赖固定台词或动作 | `通过` |
...
Milestone 8 已完成并归档为 [Milestone8.md](../Milestones/Milestone8.md)。
