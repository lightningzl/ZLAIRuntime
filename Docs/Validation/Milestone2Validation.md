...
| Dialogue Provider | 自动化：Stub/Fake Kimi；真实 Kimi 尚未验证 |
| Kimi 模型 | `kimi-k3`（真实请求尚未验证） |
| UE 外层超时 | 30 秒 |
| Provider 超时 | 默认 20 秒 |
...
| 验证 | 命令或入口 | 结果 | 证据摘要 |
| --- | --- | --- | --- |
| Python 完整测试 | `PythonService/.venv/Scripts/python -m pytest -q -p no:cacheprovider` | 通过 | 56 passed；自动移除 API Key、拒绝非本机网络、无真实 Token |
...
Milestone 2 当前未验收。只有 `M2-A01` 至 `M2-A10` 全部具有通过证据，且 [TaskBoard.md](../TaskBoard.md) 中 `M2-08` 完成后，才能将里程碑状态更新为 `已完成`。
