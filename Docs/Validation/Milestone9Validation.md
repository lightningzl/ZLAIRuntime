...
| 验收 ID | 状态 | 证据 |
| --- | --- | --- |
| `M9-A01` | `通过` | Python 197/197、UE `ZL.Social` 27/27、受影响 Target 编译成功；协议与两端 Schema/Tool 未变更 |
| `M9-A02` | `通过` | Scheduler、个人历史与默认地图 Stub 烟测证明显著事件调度、单在途、有界 Pending 与自动上限 |
| `M9-A03` | `通过` | Attack 校验自动化覆盖目标、距离、状态、冷却与拒绝；场景由受控 Action UI 提交 |
| `M9-A04` | `通过` | NpcDamage 与 Attack 自动化覆盖命中、防卫、失能、状态版本和拒绝零副作用 |
| `M9-A05` | `通过` | Stub Planner 覆盖攻击升级、近期攻击后保持距离和道歉后的受限缓和；默认地图验证实际表达/退让 |
| `M9-A06` | `通过` | PersonalDecisionContext 与 Python Context Builder 覆盖个人事实、公开历史裁剪和未感知事实过滤 |
| `M9-A07` | `通过` | 状态版本、过期/拒绝语义、计划完成触发及默认地图动作烟测通过 |
| `M9-A08` | `通过` | 气泡、名称生命/防卫状态及 Inspector 显示冲突等级、触发、调度、来源和结果码 |
| `M9-A09` | `通过` | 服务离线默认地图烟测通过：进入 `LocalFallback`/防卫且不执行 Tool 或移动 |
| `M9-A10` | `通过` | 编译、Python/UE 回归、Stub/离线烟测和真实 Kimi 默认地图烟测均通过 |
...
| 验证项 | 实际结果 |
| --- | --- |
| Python 全量回归 | 项目虚拟环境执行 `pytest --basetemp .pytest-tmp-m9`，197 项全部通过，退出码 0；临时目录随后清除。 |
| UE 全量社会回归 | NullRHI 执行 `ZL.Social`，27 项全部 `Success`，退出码 0。 |
| UE 编译 | `ZLEditor Win64 Development` 在连续调度、攻击冲突与烟测修正后编译成功。 |
| 有界与零副作用 | Scheduler、Attack、NPC Damage、ConflictState、Tool Registry 和 Personal Context 测试包含单在途/自动上限、非法攻击/受击拒绝、失能、防卫、冲突缓和和个人历史边界。 |

### `M9-T08` 真实模型验收状态

真实 Kimi Smoke 已在用户授权后执行：默认地图返回 `provider=kimi`，公开 Speech 被接受，模型未建议 Tool，Guard 的位置和状态版本保持不变；耗时 8471 ms。验收不比较固定台词或固定 Tool，且未记录任何凭据或上下文正文。
