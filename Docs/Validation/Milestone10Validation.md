...
- 2026-09-02：左侧新增默认收起的“行动与对话记录”区域，成功提交的说话或行为追加为目标与内容记录，最多保留 12 条；重置时清空。记录不包含服务响应、Prompt 或隐藏上下文。UE 5.8 `ZLEditor Win64 Development` 编译成功。
...
| `M10-A15` | 行动与对话记录默认可收起、展开后显示最多 12 条成功玩家提交，重置时清空；UE 5.8 编译通过。 |
