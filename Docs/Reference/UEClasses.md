...
| `AZLSocialSandboxPawn` | 提供玩家移动/转向、Face/Approach/MoveAway/Stop 的权威执行和说话/动作气泡，并应用预设公开显示名、颜色、出生 Transform；使用 `ACharacter::Mesh` 承载用户配置的骨骼模型/动画，并可配置 Attack/Hit Montage、Section、播放速率和 Enhanced Input | 从自由文本推断未注册行为、伪造完成状态或直接伤害 NPC |
